...
#define FPEC_BASE			0x52002000
#define BANK2_REG_OFFSET	0x100
...
#define FLASH_SR_OPERR		(1 << 22)
#define FLASH_SR_ERROR_MASK	(FLASH_SR_WRPERR  | FLASH_SR_PGSERR | \
...
#define BANK2_START (BANK1_START + NUM_SECTOR_PER_BANK * FLASH_SECTOR_SIZE)
enum ID_STM32H7 {
	ID_STM32H74x  = 0x450,
};
...
//	f->align = 32;
	f->erased = 0xff;
	sf->regbase = FPEC_BASE;
	if (addr >= BANK2_START)
		sf->regbase += BANK2_REG_OFFSET;
...
bool stm32h7_probe(target *t)
{
	...
	case ID_STM32H74x: /* H743/H573 */
		...
		/* size per bank! */
		size    = (target_mem_read32(t, FLASH_SIZE_REG) & 0xfff) << 9;
		stm32h7_add_flash(t, 0x8000000       , size, FLASH_SECTOR_SIZE);
		stm32h7_add_flash(t, 0x8000000 + size, size, FLASH_SECTOR_SIZE);
...
{
	uint32_t regbase = FPEC_BASE;
	if (addr >= BANK2_START)
		regbase += BANK2_REG_OFFSET;
	/* gdb_main calls the erase functions only after a target reset.
	 * So we run on 64 MHz HSI64 clock and can use WRHIGHFREQ 0 and read
	 * latency 1 even with small VOS. For bank/mass erase and option write,
	 * issue explicit reset to the target so HSI64 is used.*/
	target_mem_write32(t, regbase + FLASH_ACR, 1);
	while(target_mem_read32(t, regbase + FLASH_SR) & FLASH_SR_BSY) {
		if(target_check_error(t)) {
			DEBUG("FUL: comm failed\n");
			return false;
		}
	}
							   ...
							   size_t len)
{
	...
	while (end_sector >= start_sector) {
		uint32_t cr = FLASH_CR_PSIZE64 | FLASH_CR_SER |
			(end_sector * FLASH_CR_SNB_1);
		target_mem_write32(t, sf->regbase + FLASH_CR, cr);
		target_mem_write32(t, sf->regbase + FLASH_CR, cr | FLASH_CR_START);
		uint32_t sr;
		while (((sr = target_mem_read32(t, sf->regbase + FLASH_SR))
				& FLASH_SR_BSY)) {
			if(target_check_error(t)) {
                               ...
                               const void *src, size_t len)
{
	target *t = f->t;
	struct stm32h7_flash *sf = (struct stm32h7_flash *)f;
	target_mem_write32(t, sf->regbase + FLASH_CR, FLASH_CR_PSIZE64 | FLASH_CR_PG);
	/* Use RM0433 Rev 3. 3.3.7 "Optimal block write sequence"
	 * We rely on system stalling with write command queue and the write buffer full.
	 */
	...
	while ((sr = target_mem_read32(t, sr_reg)) & FLASH_SR_BSY) {
		if(target_check_error(t)) {
			DEBUG("stm32h7_flash_write: comm failed\n");
			return -1;
		}
	}
	if (sr & FLASH_SR_ERROR_MASK) {
	...
	/* Close write windows.*/
	target_mem_write32(t, sf->regbase + FLASH_CR, FLASH_CR_PSIZE64);
	return 0;
}

/* Both banks are erasen concurremt.*/
static bool stm32h7_cmd_erase(target *t, int bank_mask)
{
	...
	uint32_t cr;
	bool result = false;
	/* The clock frequency must be know for setting sensible
	 * flash parameters. With a reset we switch to HSI64 with
	 *  small effort.
	 */
	target_reset(t);
	cr = FLASH_CR_PSIZE64 | FLASH_CR_BER;
	...
	if (do_bank1) {
		...
		target_mem_write32(t, regbase + FLASH_CR, cr | FLASH_CR_START);
	}
	if (do_bank2) {
		if (stm32h7_flash_unlock(t, BANK2_START) == false)
			goto done;
		uint32_t regbase = FPEC_BASE + BANK2_REG_OFFSET;
		target_mem_write32(t, regbase + FLASH_CR, cr);
		target_mem_write32(t, regbase + FLASH_CR, cr | FLASH_CR_START);
	}
	...
	tc_printf(t, "\n");
	if (!result) {
		DEBUG("sr %" PRIx32 " cr %" PRIx32 ", sr2 %" PRIx32 ", cr2 %" PRIx32 "\n",
			  target_mem_read32(t, FPEC_BASE + FLASH_SR),
			  target_mem_read32(t, FPEC_BASE + FLASH_CR),
			  target_mem_read32(t, FPEC_BASE + BANK2_REG_OFFSET + FLASH_SR),
			  target_mem_read32(t, FPEC_BASE + BANK2_REG_OFFSET + FLASH_CR));
	}
	return result;
}
...
static bool stm32h7_crc(target *t)
{
	...
	uint32_t crc1 = target_mem_read32(t, FPEC_BASE + FLASH_CRCDATA);
	if (stm32h7_crc_bank(t, BANK2_START) ) return false;
	uint32_t crc2 = target_mem_read32(t, FPEC_BASE + FLASH_CRCDATA);
